...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between">
        ...
        <div>
            ...
            @else
                <form method="POST" action="{{ route('children.activate',$child) }}" class="d-inline">@csrf
    ...
    <div class="modal fade" id="deactivateModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('children.deactivate',$child) }}">@csrf
                    <div class="modal-header"><h5 class="modal-title">Подтверждение</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body">
                        ...
                        @php
                            ...
                            $q->where(function($w){ $w->whereNotNull('visits_left')->where('visits_left','>',0); })
                            ->orWhere(function($w){ $w->whereNull('visits_left')->where(function($w2){ $w2->whereNull('expires_at')->orWhere('expires_at','>=',now()); }); });
                            })->exists();
                        @endphp
                        @if($hasActivePaid)
                            <div class="alert alert-warning">Внимание: у ребёнка есть активный оплаченный пакет!</div>
                        @endif
                    </div>
